    ...
    <form onSubmit={handleSubmit(onSubmit)} noValidate>
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
        <div className="sm:col-span-2">
          <Input label="Job Title" required placeholder="e.g. Frontend Developer"
            error={errors.title?.message} {...register('title')} />
        </div>
        <Select label="Department" required options={DEPARTMENT_OPTIONS}
          placeholder="— Select department —"
          error={errors.department?.message} {...register('department')} />
        <Input label="Location" required placeholder="e.g. Vadodara / Remote"
          error={errors.location?.message} {...register('location')} />
        <Input label="Experience Required (years)" required type="number" min={0}
          error={errors.experience?.message}
          {...register('experience', { valueAsNumber: true })} />
        <Input label="Number of Openings" required type="number" min={1}
          error={errors.openings?.message}
          {...register('openings', { valueAsNumber: true })} />
        <Select label="Status" required options={STATUS_OPTIONS}
          error={errors.status?.message} {...register('status')} />
        <Input label="Closing Date" type="date"
          hint="Leave blank for no deadline"
          error={errors.closing_date?.message} {...register('closing_date')} />
        <div className="sm:col-span-2">
          <Textarea label="Job Description" required rows={4}
